...
export default function ProductDetails() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      // Fetch shop information if user_id is available
      if (response.data.user_id) {
        await fetchShopInfo(response.data.user_id);
      }
  ...
  const fetchShopInfo = async (userId) => {
    try {
      // First, get the seller info using user_id
      const response = await axios.get(`http://localhost:8000/api/sellers/users/${userId}/public`);
  ...
  const getImageUrl = (imagePath) => {
    if (!imagePath) return "/src/assets/placeholder.jpg";
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-baseline gap-3">
                        <p className="text-4xl lg:text-5xl font-bold text-blue-600">₱{product.price?.toLocaleString()}</p>
                        <span className="text-lg text-gray-400 line-through">₱{Math.round(product.price * 1.2)?.toLocaleString()}</span>
                        <span className="px-2 py-1 bg-green-100 text-green-700 text-sm font-semibold rounded-md">20% OFF</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-200">
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          className={`flex-1 py-4 px-6 rounded-xl font-bold text-white flex items-center justify-center gap-3 transition-all text-lg ${
                            product.stock === 0
                  ...
                  {shopInfo ? (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <h4 className="font-semibold text-gray-800 text-lg">{shopInfo.shop_name}</h4>
                      </div>
                        ...
                        {shopInfo.email && (
                          <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-gray-600 break-all">{shopInfo.email}</p>
                            </div>
                          </div>
                        )}
                        ...
                        {shopInfo.phone_number && (
                          <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600">+63 {shopInfo.phone_number}</p>
                            </div>
                          </div>
                        )}

                        {(shopInfo.house_and_ward || shopInfo.district_and_province) && (
                          <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                            <MapPin size={18} className="text-gray-600 mt-0.5 flex-shrink-0" />
                            <div className="flex-1">
                              <p className="text-sm font-medium text-gray-800">Address</p>
                              <div className="text-sm text-gray-600">
                                {shopInfo.house_and_ward && (
                                  <p>{shopInfo.house_and_ward}</p>
                                )}
                                {shopInfo.district_and_province && (
                                  <p>{shopInfo.district_and_province}</p>
                                )}
                              </div>
                            </div>
                          </div>
                      ...
                      <div className="space-y-3 pt-4 border-t border-gray-200">
                        ...
                        <button className="w-full py-3 px-4 border border-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
                          <Store size={18} />
                          Visit Shop
                        </button>
                      </div>
                      ...
                      {shopInfo.last_updated_at && (
                        <div className="pt-4 border-t border-gray-200">
                          <p className="text-xs text-gray-500">
                            Profile updated: {new Date(shopInfo.last_updated_at).toLocaleDateString('en-US', {
                              year: 'numeric',
                              month: 'short',
                              day: 'numeric'
                            })}
